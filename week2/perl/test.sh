...
top_request(){
    printf "<br/>Total Requests per status code.<br/>"
    printf "<br/>=================================<br/>"
    cat $logfile | awk '{print $7}' | sort | uniq -c > $write_file
    while read line; do
        set -- $line
        printf "<br/>Total requests on status code \"$2\" is $1.<br/>"
    done < $write_file
}
...
top_5_requested() {
    ...
    printf "<br/><br/>Top 5 Requested $name:<br/>++++++++++++++++++++++++++++++<br/>"

    while read line; do
	    set -- $line
	    printf "$name : \"$2\" requested $1 times.<br/>"
    done < $write_file
...
top_5_highest_response(){
    ...
    printf "<br/><br/>Top 5 Requested Response:<br/>+++++++++++++++++++++++<br/>"

    while read line; do
	    set -- $line
	    printf "<br/>Response Time : \"$1\" requested by: \"$2\" requested from: \"$3\" IP.<br/>"
    done < $write_file
}
...
top_5_requested_stat_by_host(){
    ...
    while read line; do
        set -- $line
        printf "<br/>Status Code : \"$2\" Host : \"$3\" requested $1 times.<br/>"
    done < $write_file
}
top_5_requested_stat_by_host
...
get_last_status_code(){
    printf "<br/>Find the time last 200/5xx/4xx was received for a particular host"
    printf "<br/>=================================================================<br/>"
    host=$1
...
request_taking_more_time(){
    printf "<br/>Requests taking more than 2 secs to respond :<br/>"
    cat $logfile | awk '{if ($8 > 2) print $8 $1 $NF}' | sort | uniq | sort | head -5
    printf '<br/><br/>'

    printf "<br/>Requests taking more than 5 secs to respond :<br/>"
    cat $logfile | awk '{if ($8 > 5) print $8 $1 $NF}' | sort | sort | uniq | sort | head -5
    printf '<br/><br/>'

    printf "<br/>Requests taking more than 10 secs to respond :<br/>"
